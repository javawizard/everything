...
public class WorkspaceManager
{
	...
	{
		WorkspaceWrapper[] fsWorkspaces = Storage.listWorkspaces();// fs stands
		// for File
		// System,
		// because
		// this is
	...
	{
		Plugin<Workspace> workspacePlugin = PluginManager.getById(fsw
				.getTypeId());
		if (workspacePlugin == null)
			throw new NullPointerException("the plugin for workspace "
					+ fsw.getId() + " isn't present on the local computer");
		Workspace workspace = workspacePlugin.create();
		fsw.setWorkspace(workspace);
		workspace.setPluginMetadata(workspacePlugin.getMetadata());
	 ...
	 * has changed.
	 * 
	 * Convergia will send pstatusupdate when a user imports a workspace, and
	 * when they delete it. Convergia will send pstatusupdate when the creator
	 * adds or removes from the allowed user list.
	 * 
	 * @param communicator
	 *            the communicator to use
	 */
	public synchronized static void init(final Communicator communicator)
	{
		...
		communicator.addSink(new MessageSink()
		{
			...
			public void process(String from, String message)
			{
				if (!message.startsWith("wsi|"))// message is not intended for
					// WorkspaceManager
					return;
				System.out.println("message received for workspace manager: "
						+ message);
				if (message.startsWith("wsi|w|"))
				{
					String remainder = message.substring("wsi|w|".length());
					int pIndex = remainder.indexOf("|");
					String workspaceId = remainder.substring(0, pIndex);
					String wsMessage = remainder.substring(pIndex + 1);
					...
					processWorkspaceSpecificMessage(from, workspace, wsMessage);
					...
					new Thread()
					{
						public void run()
						{
							if ((lastForcedReload + (1000 * 5)) < System
									.currentTimeMillis())
							{
								lastForcedReload = System.currentTimeMillis();
								reloadWorkspaceMembers();
							}
						}
					}.start();
				} else if (message.startsWith("wsi|workspaceinvite|"))// invitation
				// to a
				// workspace
				// by
				// another
				// user
				{
					System.out.println("workspace invitation received");
					try
					{
						if ((System.currentTimeMillis() - 5000) < lastInviteTime)
							return;
						System.out.println("last invite time ok");
						lastInviteTime = System.currentTimeMillis();
						String wMessage = message
								.substring("wsi|workspaceinvite|".length());
						int pIndex = wMessage.indexOf("|");
						final String wId = wMessage.substring(0, pIndex);
						String inviteText = wMessage.substring(pIndex + 1);
						inviteText = Base64Coder.decodeString(inviteText);
						final ReceiveWorkspaceInviteFrame frame = new ReceiveWorkspaceInviteFrame();
						frame.getFromLabel().setText(from);
						frame.getInviteTextArea().setText(inviteText);
						frame.getWorkspaceIdLabel().setText(wId);
						frame.getRejectButton().addActionListener(
								new ActionListener()
								{
						...
						frame.getAcceptButton().addActionListener(
								new ActionListener()
								{
		...
		Thread t1 = new Thread("WorkspaceManager user list updater")
		{
			public void run()
		...
		t1.setDaemon(true);
		t1.start();
		Thread t2 = new Thread("WorkspaceManager user status hash updater")
		{
			public void run()
			{
				while (true)
	...
	protected static void notifyUserChanged(String u, UserStatus status)
	{
		boolean isKnownUser = false;
		for (WorkspaceWrapper w : new ArrayList<WorkspaceWrapper>(workspaces))
		{
			try
			{
	...
	public static WorkspaceWrapper getById(String workspaceId)
	{
	...
	protected static void processWorkspaceSpecificMessage(String from,
			WorkspaceWrapper workspace, String wsMessage)
	{
		if (wsMessage.startsWith("imessage|"))
		{
			if (validateFromParticipant(from, workspace))
			{
				...
				if (implMessage.length() > 0)
					workspace.getWorkspace().receiveMessage(from, implMessage);
			} else
			{
				System.err
	...
	private static boolean validateFromParticipant(String from,
			WorkspaceWrapper workspace)
	{
		return workspace.getAllowedUsers().contains(from);
	}

	public static String getWorkspaceCreator(String workspaceId)
	{
		// workspace ids always start with the username that created them and
		// then a hyphen.
		return workspaceId.substring(0, workspaceId.indexOf("-"));
	}
	...
	public static void reloadWorkspaceMembers()
	{
		...
		for (WorkspaceWrapper ww : workspaces)
		{
			synchronized (ww)
			{
				try
				{
					String creator = getWorkspaceCreator(ww.getId());
					Properties creatorMetadata = mdCache.get(creator);
					if (creatorMetadata == null)
					{
						String creatorMd = communicator
								.getUserMetadata(creator);
						creatorMetadata = parseMetadata(creatorMd);
						mdCache.put(creator, creatorMetadata);
					}
					String workspaceAllowedMembersString = creatorMetadata
							.getProperty("workspace_" + ww.getId() + "_users");
					String[] workspaceAllowedMembers = null;
					if (!ww.isMine())
					{
						workspaceAllowedMembers = workspaceAllowedMembersString
								.split("\\,");
						ww.getAllowedUsers().addAll(
								Arrays.asList(workspaceAllowedMembers));
						ww.getAllowedUsers().retainAll(
								Arrays.asList(workspaceAllowedMembers));
					} else
					{
						workspaceAllowedMembers = ww.getAllowedUsers().toArray(
								new String[0]);
					}
					...
					for (String u : workspaceAllowedMembers)
					{
						try
						{
							Properties userMetadata = mdCache.get(u);
							if (userMetadata == null)
							{
								String userMd = communicator.getUserMetadata(u);
								userMetadata = parseMetadata(userMd);
								mdCache.put(u, userMetadata);
							}
							...
							String[] participantWorkspaces = participantWorkspacesString
									.split("\\,");
							if (participantWorkspacesString.trim().equals(""))
								participantWorkspaces = new String[0];
							for (String participantWorkspace : participantWorkspaces)
							{
								if (participantWorkspace.equals(ww.getId()))// this
								// user
								// is a
								// participant
								{
									if (participants.indexOf(u) == -1)
									{
										participants.add(u);
									}
								}
					...
					ww.getParticipants().addAll(participants);
					ww.getParticipants().retainAll(participants);
	...
	public static synchronized void removeWorkspace(WorkspaceWrapper w)
	{
		workspaces.remove(w);
	}
