...
/**
 * Manages the sessions associated with an account. The information maintained
 * by the Session manager is entirely transient and does not need to be
...
public class SessionManager extends BasicModule implements ClusterEventListener
{
    ...
    /**
     * Counter of user connections. A connection is counted just after it was
     * created and not after the user became available. This counter only
     * considers sessions local to this JVM. That means that when running inside
     * of a cluster you will need to add up this counter for each cluster node.
     */
    private final AtomicInteger connectionsCounter = new AtomicInteger(0);
    
    /**
     * Cache (unlimited, never expire) that holds information about client
     * sessions (as soon as a resource has been bound). The cache is used by
     * Remote sessions to avoid generating big number of remote calls. Key: full
     * JID, Value: ClientSessionInfo
     */
    ...
    /**
     * Cache (unlimited, never expire) that holds sessions of connection
     * managers. For each socket connection of the CM to the server there is
     * going to be an entry in the cache. Key: full address of the CM that
     * identifies the socket, Value: nodeID
     */
    private Cache<String, byte[]> multiplexerSessionsCache;
    
    /**
     * Cache (unlimited, never expire) that holds incoming sessions of remote
     * servers. Key: stream ID that identifies the socket/session, Value: nodeID
     */
    private Cache<String, byte[]> incomingServerSessionsCache;
    /**
     * Cache (unlimited, never expire) that holds list of incoming sessions
     * originated from the same remote server (domain/subdomain). For instance,
     * jabber.org may have 2 connections to the server running in
     * jivesoftware.com (one socket to jivesoftware.com and the other socket to
     * conference.jivesoftware.com). Key: remote hostname (domain/subdomain),
     * Value: list of stream IDs that identify each socket.
     */
    private Cache<String, List<String>> hostnameSessionsCache;
    
    /**
     * Cache (unlimited, never expire) that holds domains, subdomains and
     * virtual hostnames of the remote server that were validated with this
     * server for each incoming server session. Key: stream ID, Value: Domains
     * and subdomains of the remote server that were validated with this server.
     * <p>
     * 
     * This same information is stored in {@link LocalIncomingServerSession} but
     * the reason for this duplication is that when running in a cluster other
     * nodes will have access to this clustered cache even in the case of this
     * node going down.
     */
    ...
    private ComponentSessionListener componentSessionListener =
        new ComponentSessionListener();
    private IncomingServerSessionListener incomingServerListener =
        new IncomingServerSessionListener();
    private OutgoingServerSessionListener outgoingServerListener =
        new OutgoingServerSessionListener();
    private ConnectionMultiplexerSessionListener multiplexerSessionListener =
        new ConnectionMultiplexerSessionListener();
    
    /**
     * Local session manager responsible for keeping sessions connected to this
     * JVM that are not present in the routing table.
     */
    ...
    /**
     * Returns the instance of <CODE>SessionManagerImpl</CODE> being used by the
     * XMPPServer.
     * 
     * @return the instance of <CODE>SessionManagerImpl</CODE> being used by the
     *         XMPPServer.
     */
     ...
     * <tt>null</tt> if none was found. The specified address MUST contain a
     * resource that uniquely identifies the session.
     * 
     * A single connection manager should connect to the same node.
     * 
     * @param address
     *            the address of the connection manager (including resource that
     *            identifies specific socket)
    ...
    public ConnectionMultiplexerSession getConnectionMultiplexerSession(JID address)
    {
        ...
        if (session == null && server.getRemoteSessionLocator() != null)
        {
            ...
            if (nodeID != null)
            {
                return server.getRemoteSessionLocator()
                    .getConnectionMultiplexerSession(nodeID, address);
            }
        }
        return null;
    }
    ...
    public List<ConnectionMultiplexerSession> getConnectionMultiplexerSessions()
    {
        List<ConnectionMultiplexerSession> sessions =
            new ArrayList<ConnectionMultiplexerSession>();
        ...
        if (locator != null)
        {
            for (Map.Entry<String, byte[]> entry : multiplexerSessionsCache.entrySet())
            {
                ...
                {
                    sessions.add(locator.getConnectionMultiplexerSession(entry
                        .getValue(), new JID(entry.getKey())));
                }
            }
        }
        return sessions;
    ...
    /**
     * Returns a collection with all the sessions originated from the connection
     * manager whose domain matches the specified domain. If there is no
     * connection manager with the specified domain then an empty list is going
     * to be returned.
     * 
     * @param domain
     *            the domain of the connection manager.
     * @return a collection with all the sessions originated from the connection
     *         manager whose domain matches the specified domain.
     */
    public List<ConnectionMultiplexerSession> getConnectionMultiplexerSessions(
        String domain)
    {
        List<ConnectionMultiplexerSession> sessions =
            new ArrayList<ConnectionMultiplexerSession>();
        // Add sessions of CMs connected to this JVM
        for (String address : localSessionManager.getConnnectionManagerSessions()
            .keySet())
        {
            JID jid = new JID(address);
            if (domain.equals(jid.getDomain()))
            {
                sessions.add(localSessionManager.getConnnectionManagerSessions().get(
                    address));
            }
        }
        // Add sessions of CMs connected to other cluster nodes
     ...
     * @param address
     *            the JID (may include a resource) of the connection manager's
     *            session.
        ...
        JID address)
    {
        ...
        LocalConnectionMultiplexerSession session =
            new LocalConnectionMultiplexerSession(serverName, conn, id);
        conn.init(session);
        // Register to receive close notification on this session so we can
        // figure out when users that were using this connection manager may
        // become unavailable
        conn.registerCloseListener(multiplexerSessionListener, session);
        
        // Add to connection multiplexer session.
        boolean firstConnection =
            getConnectionMultiplexerSessions(address.getDomain()).isEmpty();
        localSessionManager.getConnnectionManagerSessions().put(address.toString(),
            session);
        ...
        if (firstConnection)
        {
            ...
            // is available
            ConnectionMultiplexerManager.getInstance().multiplexerAvailable(
                address.getDomain());
        }
        return session;
    }
    ...
    /**
     * Creates a new <tt>ClientSession</tt>. The new Client session will have a
     * newly created stream ID.
     * 
     * @param conn
     *            the connection to create the session from.
     * @return a newly created session.
    ...
    public LocalClientSession createClientSession(Connection conn, StreamID id)
    {
        ...
        localSessionManager.getPreAuthenticatedSessions().put(
            session.getAddress().getResource(), session);
        // Increment the counter of user sessions
        connectionsCounter.incrementAndGet();
        return session;
    }
    
    public HttpSession createClientHttpSession(long rid, InetAddress address,
        StreamID id, HttpConnection connection) throws UnauthorizedException
    {
        if (serverName == null)
        {
        ...
        HttpSession session =
            new HttpSession(backupDeliverer, serverName, address, id, rid, connection);
    ...
    public LocalComponentSession createComponentSession(JID address, Connection conn)
    {
        ...
        componentSessionsCache
            .put(address.toString(), server.getNodeID().toByteArray());
        return session;
    }
    
    /**
     * Creates a session for a remote server. The session should be created only
     * after the remote server has been authenticated either using
     * "server dialback" or SASL.
     * 
     * @param conn
     *            the connection to the remote server.
     * @param id
     *            the stream ID used in the stream element when authenticating
     *            the server.
    ...
    public LocalIncomingServerSession createIncomingServerSession(Connection conn,
        StreamID id) throws UnauthorizedException
    {
        if (serverName == null)
        {
            throw new UnauthorizedException("Server not initialized");
        ...
        LocalIncomingServerSession session =
            new LocalIncomingServerSession(serverName, conn, id);
        conn.init(session);
        // Register to receive close notification on this session so we can
     ...
     * Register a listener that will react when the connection gets closed.
     * 
     * @param session
     ...
     * a given hostname. Notice that the remote server may be hosting several
     * subdomains as well as virtual hosts so the same IncomingServerSession may
     * be associated with many keys. If the remote server creates many sessions
     * to this server (eg. one for each subdomain) then associate all the
     * sessions with the originating server that created all the sessions.
     * 
     * @param hostname
     *            the hostname that is being served by the remote server.
     * @param session
    ...
    /**
     * Unregisters the specified remote server session originiated by the
     * specified remote server.
     * 
     * @param hostname
     *            the hostname that is being served by the remote server.
     * @param session
     *            the session to unregiser.
     */
    public void unregisterIncomingServerSession(String hostname,
        IncomingServerSession session)
    {
        // Remove local track of the incoming server session connected to this
        // JVM
        String streamID = session.getStreamID().getID();
    ...
    /**
     * Returns a collection with all the domains, subdomains and virtual hosts
     * that where validated. The remote server is allowed to send packets from
     * any of these domains, subdomains and virtual hosts.
     * <p>
     * 
     * Content is stored in a clustered cache so that even in the case of the
     * node hosting the sessions is lost we can still have access to this info
     * to be able to perform proper clean up logic.
     * 
     * @param streamID
    ...
    /**
     * Add a new session to be managed. The session has been authenticated and
     * resource binding has been done.
     * 
     * @param session
    ...
    {
        // Remove the pre-Authenticated session but remember to use the
        // temporary ID as the key
        localSessionManager.getPreAuthenticatedSessions().remove(
            session.getStreamID().toString());
        // Add session to the routing table (routing table will know session is
        // not available yet)
        routingTable.addClientRoute(session.getAddress(), session);
        SessionEventDispatcher.EventType event =
            session.getAuthToken().isAnonymous() ? SessionEventDispatcher.EventType.anonymous_session_created
                : SessionEventDispatcher.EventType.session_created;
        // Fire session created event.
        SessionEventDispatcher.dispatchEvent(session, event);
        if (ClusterManager.isClusteringStarted())
        {
            // Track information about the session and share it with other
            // cluster nodes
            sessionInfoCache.put(session.getAddress().toString(),
                new ClientSessionInfo(session));
        }
    }
    ...
    public void sessionAvailable(LocalClientSession session)
    {
        ...
        {
            // Anonymous session always have resources so we only need to add
            // one route. That is
    ...
    /**
     * Sends the presences of other connected resources to the resource that
     * just connected.
     * 
     * @param session
    ...
    private void broadcastPresenceOfOtherResource(LocalClientSession session)
    {
        ...
        for (JID address : addresses)
        {
            ...
            // Send the presence of an existing session to the session that has
            // just changed
            // the presence
    ...
    /**
     * Broadcasts presence updates from the originating user's resource to any
     * of the user's existing available resources (if any).
     * 
     * @param originatingResource
     *            the full JID of the session that sent the presence update.
     * @param presence
     *            the presence.
     */
        ...
        Presence presence)
    {
        // Get list of sessions of the same user
        JID searchJID =
            new JID(originatingResource.getNode(), originatingResource.getDomain(),
                null);
        List<JID> addresses = routingTable.getRoutes(searchJID, null);
        for (JID address : addresses)
        {
    ...
    /**
     * Notification message sent when a client sent an unavailable presence for
     * the session. Making the session unavailable means that the session is not
     * eligible for receiving messages from other clients.
     * 
     * @param session
     ...
     * 
     * @param session
     *            The session whose presence priority has been modified
     * @param oldPriority
     *            The old priority for the session
     */
    public void changePriority(LocalClientSession session, int oldPriority)
    {
        System.out.println("Attempting priority change, old = " + oldPriority);
        if (session.getAuthToken().isAnonymous())
        {
        ...
        {
            // Do nothing if new presence priority is not positive and old
            // presence negative
            return;
        }
        ...
        System.out
            .println("Got past initial priority checks, probably going to deliver messages (which is a bad thing)");
        // User sessions had negative presence before this change so deliver
        // messages
        if (session.canFloodOfflineMessages())
        {
            OfflineMessageStore messageStore = server.getOfflineMessageStore();
            System.err.println("ERROR: delivering offline messages as a result "
                + "of a priority change, this generall shouldn't happen");
            Collection<OfflineMessage> messages =
                messageStore.getMessages(session.getAuthToken().getUsername(), false);
    ...
    public boolean isActiveRoute(String username, String resource)
    {
        ...
        Session session =
            routingTable.getClientRoute(new JID(username, serverName, resource));
    ...
    /**
     * Returns the session responsible for this JID data. The returned Session
     * may have never sent an available presence (thus not have a route) or
     * could be a Session that hasn't authenticated yet (i.e.
     * preAuthenticatedSessions).
     * 
     * @param from
    ...
    {
        // Return null if the JID is null or belongs to a foreign server. If the
        // server is
        // shutting down then serverName will be null so answer null too in this
        // case.
        ...
        if (from.getResource() != null)
        {
            ClientSession session =
                localSessionManager.getPreAuthenticatedSessions().get(
                    from.getResource());
            if (session != null)
            {
                return session;
    ...
    /**
     * Returns a list that contains all authenticated client sessions connected
     * to the server. The list contains sessions of anonymous and non-anonymous
     * users.
     * 
    ...
    public Collection<ClientSession> getSessions(SessionResultFilter filter)
    {
        ...
        if (filter != null)
        {
            ...
            // doing the rest of the filtering without locking out session
            // access
            ...
            // Now generate the final list. I believe it's faster to to build up
            // a new
            // list than it is to remove items from head and tail of the sorted
            // tree
    ...
    /**
     * Returns the incoming server session hosted by this JVM that matches the
     * specified stream ID.
     * 
     * @param streamID
     *            the stream ID that identifies the incoming server session
     *            hosted by this JVM.
     * @return the incoming server session hosted by this JVM or null if none
     *         was found.
     */
    ...
    /**
     * Returns the list of sessions that were originated by a remote server. The
     * list will be ordered chronologically. IncomingServerSession can only
     * receive packets from the remote server but are not capable of sending
     * packets to the remote server.
     * 
     * @param hostname
    ...
    public List<IncomingServerSession> getIncomingServerSessions(String hostname)
    {
        ...
        else
        {
            ...
            List<IncomingServerSession> sessions =
                new ArrayList<IncomingServerSession>();
            for (String streamID : streamIDs)
            {
                // Search in local hosted sessions
                IncomingServerSession session =
                    localSessionManager.getIncomingServerSession(streamID);
                RemoteSessionLocator locator = server.getRemoteSessionLocator();
    ...
    /**
     * Returns a session that was originated from this server to a remote
     * server. OutgoingServerSession an only send packets to the remote server
     * but are not capable of receiving packets from the remote server.
     * 
     * @param hostname
     *            the name of the remote server.
     * @return a session that was originated from this server to a remote
     *         server.
     */
    ...
    public Collection<ClientSession> getSessions(String username)
    {
        ...
        {
            List<JID> addresses =
                routingTable.getRoutes(new JID(username, serverName, null, true), null);
            for (JID address : addresses)
            {
     ...
     * Sessions that are authenticated and not authenticated will be included
     * 
     * @param onlyLocal
     *            true if only sessions connected to this JVM will be
     *            considered. Otherwise count cluster wise.
    ...
    public int getConnectionsCount(boolean onlyLocal)
    {
        ...
        if (!onlyLocal)
        {
            Collection<Object> results =
                CacheFactory.doSynchronousClusterTask(new GetSessionsCountTask(false),
                    false);
     ...
     * This includes anonymous and non-anoymous users.
     * 
     * @param onlyLocal
     *            true if only sessions connected to this JVM will be
     *            considered. Otherwise count cluster wise.
    ...
    public int getUserSessionsCount(boolean onlyLocal)
    {
        ...
        {
            Collection<Object> results =
                CacheFactory.doSynchronousClusterTask(new GetSessionsCountTask(true),
                    false);
            for (Object result : results)
            {
                if (result == null)
    ...
    /**
     * Returns number of sessions coming from remote servers. <i>Current
     * implementation is only counting sessions connected to this JVM and not
     * adding up sessions connected to other cluster nodes.</i>
     * 
     * @param onlyLocal
     *            true if only sessions connected to this JVM will be
     *            considered. Otherwise count cluster wise.
    ...
    /**
     * Returns the number of sessions for a user that are available. For the
     * count of all sessions for the user, including sessions that are just
     * starting or closed.
     * 
    ...
    {
        return routingTable.getRoutes(new JID(username, serverName, null, true), null)
            .size();
    }
    ...
    /**
     * Returns a collection with the established sessions from external
     * components.
     * 
     * @return a collection with the established sessions from external
     *         components.
     */
    public Collection<ComponentSession> getComponentSessions()
    {
        ...
        if (locator != null)
        {
            for (Map.Entry<String, byte[]> entry : componentSessionsCache.entrySet())
            {
                if (!server.getNodeID().equals(entry.getValue()))
                {
                    sessions.add(locator.getComponentSession(entry.getValue(), new JID(
                        entry.getKey())));
                }
            }
        }
    ...
    /**
     * Returns the session of the component whose domain matches the specified
     * domain.
     * 
     * @param domain
     *            the domain of the component session to look for.
     * @return the session of the component whose domain matches the specified
     *         domain.
     */
    ...
    /**
     * Returns a collection with the hostnames of the remote servers that
     * currently have an incoming server connection to this server.
     * 
     * @return a collection with the hostnames of the remote servers that
     *         currently have an incoming server connection to this server.
     */
    ...
    /**
     * Returns a collection with the hostnames of the remote servers that
     * currently may receive packets sent from this server.
     * 
     * @return a collection with the hostnames of the remote servers that
     *         currently may receive packets sent from this server.
     */
    ...
    /**
     * Broadcasts the given data to all connected sessions. Excellent for server
     ...
     * 
     * @param username
     *            the user to send the boradcast to.
     * @param packet
     *            the packet to be broadcast.
    ...
    public boolean removeSession(LocalClientSession session)
    {
        ...
        // Consider session anonymous (for this matter) if we are closing a
     ...
     * @param forceUnavailable
     *            true if an unavailable presence must be created and routed.
     * @return true if the requested session was successfully removed.
     */
    public boolean removeSession(ClientSession session, JID fullJID, boolean anonymous,
        boolean forceUnavailable)
    {
        ...
        // Remove the session from the pre-Authenticated sessions list (if
        // present)
        boolean preauth_removed =
            localSessionManager.getPreAuthenticatedSessions().remove(
                fullJID.getResource()) != null;
        ...
        // Stop tracking information about the session and share it with other
        // cluster nodes
    ...
    /**
     * Returns the temporary keys used by the sessions that has not been
     * authenticated yet. This is an utility method useful for debugging
     * situations.
     * 
     * @return the temporary keys used by the sessions that has not been
     *         authenticated yet.
     */
    ...
    /**
     * Returns true if the specified address belongs to a preauthenticated
     * session. Preauthenticated sessions are only available to the local
     * cluster node when running inside of a cluster.
     * 
     * @param address
     *            the address of the session.
     * @return true if the specified address belongs to a preauthenticated
     *         session.
     */
    public boolean isPreAuthenticatedSession(JID address)
    {
        return serverName.equals(address.getDomain())
            && localSessionManager.getPreAuthenticatedSessions().containsKey(
                address.getResource());
    }
    ...
    {
        conflictLimit = limit;
        JiveGlobals.setProperty("xmpp.session.conflict-limit", Integer
            .toString(conflictLimit));
    }
        ...
        public void onConnectionClose(Object handback)
        {
            try
            {
                ...
                try
                {
                    ...
                    {
                        // Send an unavailable presence to the user's
                        // subscribers
                        // Note: This gives us a chance to send an unavailable
                        // presence to the
    ...
    private class ComponentSessionListener implements ConnectionCloseListener
    {
        ...
        public void onConnectionClose(Object handback)
        {
            ...
            try
            {
                ...
                {
                    String subdomain =
                        domain.substring(0, domain.indexOf(serverName) - 1);
                    InternalComponentManager.getInstance().removeComponent(subdomain,
                        session.getExternalComponent());
                }
            }
            catch (Exception e)
            {
            ...
            finally
            {
                ...
                // Remove track of the cluster node hosting the external
                // component
                // if no more components are handling it.
                if (!InternalComponentManager.getInstance().hasComponent(
                    session.getAddress()))
                {
    ...
    private class IncomingServerSessionListener implements ConnectionCloseListener
    {
        ...
        public void onConnectionClose(Object handback)
        {
            ...
            // Remove all the hostnames that were registered for this server
            // session
    ...
    private class ConnectionMultiplexerSessionListener implements
        ConnectionCloseListener
    {
        ...
        public void onConnectionClose(Object handback)
        {
            ConnectionMultiplexerSession session =
                (ConnectionMultiplexerSession) handback;
            // Remove all the hostnames that were registered for this server
            // session
            String domain = session.getAddress().getDomain();
            localSessionManager.getConnnectionManagerSessions().remove(
            ...
            {
                // Terminate ClientSessions originated from this connection
                // manager
                // that are still active since the connection manager has gone
                // down
                ConnectionMultiplexerManager.getInstance().multiplexerUnavailable(
                    domain);
            }
        }
    }
    ...
    public void initialize(XMPPServer server)
    {
        ...
        String conflictLimitProp =
            JiveGlobals.getProperty("xmpp.session.conflict-limit");
    ...
    /**
     * Sends a message with a given subject and body to all the active user
     * sessions in the server.
     * 
     * @param subject
     *            the subject to broadcast.
     * @param body
    ...
    /**
     * Sends a message with a given subject and body to one or more user
     * sessions related to the specified address. If address is null or the
     * address's node is null then the message will be sent to all the user
     * sessions. But if the address includes a node but no resource then the
     * message will be sent to all the user sessions of the requeted user
     * (defined by the node). Finally, if the address is a full JID then the
     * message will be sent to the session associated to the full JID. If no
     * session is found then the message is not sent.
     * 
     * @param address
     *            the address that defines the sessions that will receive the
     *            message.
     * @param subject
     *            the subject to broadcast.
     * @param body
    ...
    public void stop()
    {
        ...
        {
            sendServerMessage(null, LocaleUtils
                .getLocalizedString("admin.shutdown.now"));
        }
    ...
    /**
     * Returns true if remote servers are allowed to have more than one
     * connection to this server. Having more than one connection may improve
     * number of packets that can be transfered per second. This setting only
     * used by the server dialback mehod.
     * <p>
     * 
     * It is highly recommended that {@link #getServerSessionTimeout()} is
     * enabled so that dead connections to this server can be easily discarded.
     * 
     * @return true if remote servers are allowed to have more than one
     *         connection to this server.
     */
    public boolean isMultipleServerConnectionsAllowed()
    {
        return JiveGlobals
            .getBooleanProperty("xmpp.server.session.allowmultiple", true);
    }
    
    /**
     * Sets if remote servers are allowed to have more than one connection to
     * this server. Having more than one connection may improve number of
     * packets that can be transfered per second. This setting only used by the
     * server dialback mehod.
     * <p>
     * 
     * It is highly recommended that {@link #getServerSessionTimeout()} is
     * enabled so that dead connections to this server can be easily discarded.
     * 
     * @param allowed
     *            true if remote servers are allowed to have more than one
     *            connection to this server.
     */
    public void setMultipleServerConnectionsAllowed(boolean allowed)
    {
        JiveGlobals.setProperty("xmpp.server.session.allowmultiple", Boolean
            .toString(allowed));
        if (allowed
            && JiveGlobals.getIntProperty("xmpp.server.session.idle", 10 * 60 * 1000) <= 0)
        {
            Log
                .warn("Allowing multiple S2S connections for each domain, without setting a "
     ...
     *****************************************************/
    /**
     * Sets the number of milliseconds to elapse between clearing of idle server
     * sessions.
     * 
     * @param timeout
    ...
    public void setServerSessionTimeout(int timeout)
    {
        ...
        JiveGlobals.setProperty("xmpp.server.session.timeout", Integer
            .toString(timeout));
    }
    
    /**
     * Returns the number of milliseconds to elapse between clearing of idle
     * server sessions.
     * 
     * @return the number of milliseconds to elapse between clearing of idle
     *         server sessions.
     */
    ...
    {
        restoreCacheContent();
        // Track information about local sessions and share it with other
        // cluster nodes
        for (ClientSession session : routingTable.getClientsRoutes(true))
        {
            sessionInfoCache.put(session.getAddress().toString(),
                new ClientSessionInfo((LocalClientSession) session));
        }
    }
    ...
    {
        // Add external component sessions hosted locally to the cache (using
        // new nodeID)
        for (Session session : localSessionManager.getComponentsSessions())
        {
            componentSessionsCache.put(session.getAddress().toString(), server
                .getNodeID().toByteArray());
        }
        
        // Add connection multiplexer sessions hosted locally to the cache
        // (using new nodeID)
        for (String address : localSessionManager.getConnnectionManagerSessions()
            .keySet())
        {
        ...
        // Add incoming server sessions hosted locally to the cache (using new
        // nodeID)
        for (LocalIncomingServerSession session : localSessionManager
            .getIncomingServerSessions())
        {
            ...
            for (String hostname : session.getValidatedDomains())
            {
                // Update list of sockets/sessions coming from the same remote
                // hostname
                Lock lock = CacheFactory.getLock(hostname, hostnameSessionsCache);
                try
                {
