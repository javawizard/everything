...
/**
 * Implements JEP-0013: Flexible Offline Message Retrieval. Allows users to
 * request number of messages, request message headers, retrieve specific
 * messages, remove specific messages, retrieve all messages and remove all
 * messages.
 * 
 * @author Gaston Dombiak
 */
public class IQOfflineMessagesHandler extends IQHandler implements
    ServerFeaturesProvider, DiscoInfoProvider, DiscoItemsProvider
{
    ...
    public IQ handleIQ(IQ packet) throws UnauthorizedException
    {
        ...
        {
            // Mark that offline messages shouldn't be sent when the user
            // becomes available
            stopOfflineFlooding(from);
            // User requested to receive all offline messages
            for (OfflineMessage offlineMessage : messageStore.getMessages(from
                .getNode(), false))
            {
            ...
            for (Iterator it = offlineRequest.elementIterator("item"); it.hasNext();)
            {
                ...
                if (!"rcontent".equals(itemAction))
                {
                    synchronized (dateFormat)
                    {
                        ...
                        {
                            creationDate =
                                dateFormat.parse(item.attributeValue("node"));
                        }
                ...
                if ("view".equals(item.attributeValue("action")))
                {
                    ...
                    OfflineMessage offlineMsg =
                        messageStore.getMessage(from.getNode(), creationDate);
                    if (offlineMsg != null)
                    {
                ...
                else if ("rcontent".equals(itemAction))
                {
                    messageStore.deleteMessageByContent(from.getNode(), item
                        .attributeValue("node"));
                }
            }
        }
    ...
    {
        // Mark that offline messages shouldn't be sent when the user becomes
        // available
        ...
        field.addValue(String.valueOf(messageStore.getMessages(senderJID.getNode(),
            false).size()));
        dataForm.addField(field);
    ...
    public boolean hasInfo(String name, String node, JID senderJID)
    {
        return NAMESPACE.equals(node)
            && userManager.isRegisteredUser(senderJID.getNode());
    }
    ...
    public Iterator<DiscoItem> getItems(String name, String node, JID senderJID)
    {
        ...
        for (OfflineMessage offlineMessage : messageStore.getMessages(senderJID
            .getNode(), false))
        {
            synchronized (dateFormat)
            {
                answer.add(new DiscoItem(new JID(senderJID.toBareJID()), offlineMessage
                    .getFrom().toString(), dateFormat.format(offlineMessage
    ...
    private void stopOfflineFlooding(JID senderJID)
    {
        LocalClientSession session =
            (LocalClientSession) sessionManager.getSession(senderJID);
        if (session != null)
        {
