...
public class OfflineMessageStrategy extends BasicModule
{
    ...
    private static List<OfflineMessageListener> listeners =
        new CopyOnWriteArrayList<OfflineMessageListener>();
     ...
     * 
     * @param message
     * @return The id of the message stored, or -1 if it wasn't stored
     */
    public long storeOffline(Message message)
    {
        if (message != null)
        {
            // Do nothing if the message was sent to the server itself, an
            // anonymous user or a non-existent user
            ...
            // Do not store messages of type groupchat, error or headline as
            // specified in JEP-160
            ...
            PrivacyList list =
                PrivacyListManager.getInstance().getDefaultPrivacyList(
    ...
    {
        return quota > messageStore.getSize(message.getTo().getNode())
            + message.toXML().length();
    }
    
    private long store(Message message)
    {
        long mid = messageStore.addMessage(message);
    ...
    public enum Type
    {
        ...
        /**
         * Messages are stored up to the storage limit, and then silently
         * dropped.
         */
        store_and_drop
    }
