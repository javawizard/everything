...
/**
 * Implements the presence protocol. Clients use this protocol to update
 * presence and roster information.
 * <p/>
 * The handler must properly detect the presence type, update the user's roster,
 * and inform presence subscribers of the session's updated presence status.
 * Presence serves many purposes in Jabber so this handler will likely be the
 * most complex of all handlers in the server.
 * <p/>
 * There are four basic types of presence updates:
 * <ul>
 * <li>Simple presence updates - addressed to the server (or to address), these
 * updates are properly addressed by the server, and multicast to interested
 * subscribers on the user's roster. An empty, missing, or "unavailable" type
 * attribute indicates a simple update (there is no "available" type although it
 * should be accepted by the server.
 * <li>Directed presence updates - addressed to particular jabber entities,
 * these presence updates are properly addressed and directly delivered to the
 * entity without broadcast to roster subscribers. Any update type is possible
 * except those reserved for subscription requests.
 ...
 * </ul>
 * The valid types include "subscribe", "subscribed", "unsubscribed", and
 * "unsubscribe".
 * <li>XMPPServer probes - Provides a mechanism for servers to query the
 * presence status of users on another server. This allows users to immediately
 * know the presence status of users when they come online rather than way for a
 * presence update broadcast from the other server or tracking them as they are
 * received. Requires S2S capabilities.
 * </ul>
 * 
    ...
    ClusterEventListener
{
    ...
    /**
     * Keeps track of entities that sent directed presences to other entities.
     * In this map we keep track of every directed presence no matter if the
     * recipient was hosted in this JVM or another cluster node.
     * 
     * Key: sender, Value: list of DirectedPresences
     */
    private Cache<String, Collection<DirectedPresence>> directedPresencesCache;
    /**
     * Same as the directedPresencesCache but only keeps directed presences sent
     * from users connected to this JVM.
     */
    ...
    public PresenceUpdateHandler()
    {
        ...
        localDirectedPresences =
            new ConcurrentHashMap<String, Collection<DirectedPresence>>();
    }
        ...
        throws UnauthorizedException, PacketException
    {
        try
        {
            ...
            if (type == null)
            {
                ...
                // Notify the presence manager that the user is now available.
                // The manager may
                // remove the last presence status sent by the user when he went
                // offline.
            ...
            else if (Presence.Type.unavailable == type)
            {
                ...
                // Notify the presence manager that the user is now unavailable.
                // The manager may
                // save the last presence status sent by the user and keep track
                // when the user
                // went offline.
            ...
            else
            {
                ...
                {
                    presence
                        .setFrom(new JID(null, session.getServerName(), null, true));
        ...
        catch (Exception e)
        {
            Log.error(LocaleUtils.getLocalizedString("admin.error")
                + ". Triggered by packet: " + presence, e);
        }
    }
        ...
        catch (UnauthorizedException e)
        {
            try
            {
                LocalSession session =
                    (LocalSession) sessionManager.getSession(presence.getFrom());
                presence = presence.createCopy();
                if (session != null)
                {
                    presence
     ...
     *             If the user being updated does not exist
     */
    private void initSession(final ClientSession session) throws UserNotFoundException
    {
        ...
        if (userManager.isRegisteredUser(session.getAddress().getNode()))
        {
            ...
            // Send pending subscription requests to user if roster service is
            // enabled
            if (RosterManager.isRosterServiceEnabled())
            {
                ...
                for (RosterItem item : roster.getRosterItems())
                {
                    ...
                    {
                        session.process(createSubscribePresence(item.getJid(), new JID(
                            session.getAddress().toBareJID()), true));
                    }
                    else if (item.getRecvStatus() == RosterItem.RECV_UNSUBSCRIBE)
                    {
                        session.process(createSubscribePresence(item.getJid(), new JID(
                            session.getAddress().toBareJID()), false));
                    }
                    if (item.getSubStatus() == RosterItem.SUB_TO
                        || item.getSubStatus() == RosterItem.SUB_BOTH)
                    {
                        presenceManager.probePresence(session.getAddress(), item
                            .getJid());
                    }
                }
            }
            ...
            {
                // deliver offline messages if any. We'll do this in a new
                // thread, since it could take a while if there are a lot of new
                // messages. This might occasionally cause a message to be
                // delivered twice (if it's sent while this thread is running,
                // then it will get delivered as an offline message and as an
                // immediate message), but the client will keep a table of
                // message ids that it has received, which it can check all
                // incoming messages against.
                new Thread()
                {
                    public void run()
                    {
                        System.out.println("session is class "
                            + session.getClass().getName());
                        long messageId = messageStore.loadNextMessageId(username, 0);
                        while (messageId != -1)
                        {
                            OfflineMessage message =
                                messageStore.getMessageById(username, messageId);
                            if (message == null)
                                System.err.println("Offline message for id "
                                    + messageId + " was null, this is an error");
                            else
                                session.process(message);
                            messageId =
                                messageStore.loadNextMessageId(username, messageId);
                            if (session instanceof LocalSession)
                            {
                                LocalSession localSession = (LocalSession) session;
                                if (localSession.getConnection() == null
                                    || localSession.getConnection().isClosed())
                                {
                                    System.out
                                        .println("breaking offline message delivery at "
                                            + messageId + " on closed session");
                                    break;
                                }
                            }
                        }
    ...
    public Presence createSubscribePresence(JID senderAddress, JID targetJID,
        boolean isSubscribe)
    {
    ...
    private void broadcastUpdate(Presence update)
    {
        ...
        else
        {
            ...
            Log.warn("Presence requested from server "
                + localServer.getServerInfo().getXMPPDomain() + " by unknown user: "
                + update.getFrom());
        }
    }
     ...
     * presence to an entity. If the sender of the presence is local (to this
     * server) and the target entity does not belong to the user's roster then
     * update the registry of sent directed presences by the user.
     * 
     * @param update
     *            the directed Presence sent by the user to an entity.
     * @param handlerJID
     *            the JID of the handler that will receive/handle/process the
     *            sent packet.
     * @param jid
    ...
    public void directedPresenceSent(Presence update, JID handlerJID, String jid)
    {
        ...
        if (localServer.isLocal(update.getFrom()))
        {
            ...
            if (name != null && !"".equals(name))
            {
                // Keep track of all directed presences if roster service is
                // disabled
                if (!RosterManager.isRosterServiceEnabled())
                {
                    keepTrack = true;
                ...
                else
                {
                    ...
                    {
                        Roster roster = rosterManager.getRoster(name);
                        // If the directed presence was sent to an entity that
                        // is not in the user's
                        // roster, keep a registry of this so that when the user
                        // goes offline we
                        // will be able to send the unavailable presence to the
                        // entity
            ...
            if (keepTrack)
            {
                ...
                try
                {
                    ...
                    Collection<DirectedPresence> directedPresences =
                        directedPresencesCache.get(sender);
                    if (Presence.Type.unavailable.equals(update.getType()))
                    {
                        if (directedPresences != null)
                        {
                            ...
                            {
                                // Client sessions will receive only presences
                                // to the same JID (the
                                // address of the session) so remove the handler
                                // from the map
                                for (DirectedPresence directedPresence : directedPresences)
                                {
                                    if (directedPresence.getHandler()
                                        .equals(handlerJID))
                                    {
                            ...
                            {
                                // A service may receive presences for many JIDs
                                // so in this case we
                                // just need to remove the jid that has received
                                // a directed
                            ...
                            {
                                // Remove the user from the registry since the
                                // list of directed
                                // presences is empty
                    ...
                    else
                    {
                        if (directedPresences == null)
                        {
                            // We are using a set to avoid duplicate jids in
                            // case the user
                            // sends several directed presences to the same
                            // handler. The Map also
                            // ensures that if the user sends several presences
                            // to the same handler
                            // we will have only one entry in the Map
                            directedPresences =
                                new ConcurrentLinkedQueue<DirectedPresence>();
                        }
                        // Add the handler to the list of handler that processed
                        // the directed
                        // presence sent by the user. This handler will be used
                        // to send
    ...
    /**
     * Sends an unavailable presence to the entities that received a directed
     * (available) presence by the user that is now going offline.
     * 
     * @param update
     *            the unavailable presence sent by the user.
     */
    private void broadcastUnavailableForDirectedPresences(Presence update)
    {
        ...
        if (localServer.isLocal(from))
        {
            ...
            if (directedPresences != null)
            {
                // Iterate over all the entities that the user sent a directed
                // presence
                for (DirectedPresence directedPresence : directedPresences)
                {
    ...
    public boolean hasDirectPresence(JID ownerJID, JID recipientJID)
    {
        ...
        Collection<DirectedPresence> directedPresences =
            directedPresencesCache.get(ownerJID.toString());
        if (directedPresences != null)
        {
    ...
    public void removedExpiredPresences()
    {
        ...
        for (Map.Entry<String, Collection<DirectedPresence>> entry : copy.entrySet())
        {
            for (DirectedPresence directedPresence : entry.getValue())
            {
                ...
                {
                    Collection<DirectedPresence> presences =
                        localDirectedPresences.get(entry.getKey());
    ...
    public void initialize(XMPPServer server)
    {
        ...
        // TODO Add as route listener (to remove direct presences info for
        // removed routes). Mainly for c2s sessions which is uncommon.
    ...
    {
        // Populate directedPresencesCache with local content since when not in
        // a cluster
        // we could still send directed presences to entities that when
        // connected to a cluster
        // they will be replicated. An example would be MUC rooms.
        for (Map.Entry<String, Collection<DirectedPresence>> entry : localDirectedPresences
            .entrySet())
        {
            if (entry.getValue().isEmpty())
            {
                Log
                    .warn("PresenceUpdateHandler - Skipping empty directed presences when joining cluster for sender: "
                        + entry.getKey());
                continue;
            }
    ...
    public void leftCluster()
    {
        if (!XMPPServer.getInstance().isShuttingDown())
        {
                ...
                .entrySet())
            {
                ...
                {
                    Log
                        .warn("PresenceUpdateHandler - Skipping empty directed presences when leaving cluster for sender: "
                            + entry.getKey());
                    continue;
                }
                
                // TODO perhaps we should not lock for every entry. Instead,
                // lock it
                // once (using a LOCK_ALL global key), and handle iterations in
                // one go. We should first make sure that this doesn't lead to
                // deadlocks though! The tryLock() mechanism could be used to
                // first
                // try one approach, but fall back on the other approach.
                Lock lock =
                    CacheFactory.getLock(entry.getKey(), directedPresencesCache);
                try
                {
                    lock.lock();
                    directedPresencesCache.put(entry.getKey(), entry.getValue());
