...
public class IRCScreen extends VerticalScreen
{
    ...
    protected void select1(IrcBot[] result)
    {
        ...
        {
            Label label = new Label(bot.getNick() + "@" + bot.getServer() + ":"
                    + bot.getPort() + "/" + bot.getChannel());
            label.setTitle("Id: " + bot.getBotid());
            table.setWidget(row, 0, label);
            Anchor editLink = new Anchor("edit");
            editLink
                    .setTitle("Opens up a box that you can use to edit this IRC bot.");
            ...
            deleteLink
                    .setTitle("Deletes this IRC bot. Any triggers that use this bot "
                            + "as the recipient will be deleted, and the bot will be "
                            + "disconnected from the IRC server that it is currently "
                            + "connected to.");
        ...
        resyncButton
                .setTitle("Disconnects and then re-connects all IRC bots. This can be "
                        + "useful when the connected IRC bots have somehow gotten out "
                        + "of sync with the actual list of IRC bots.");
    ...
    protected void showUpdateBotDialog(final IrcBot bot)
    {
        ...
        panel.add(new Header3(isAdding ? "Add a new IRC bot"
                : "Editing an IRC bot"));
        FlexTable table = new FlexTable();
        panel.add(table);
        FlexCellFormatter format = table.getFlexCellFormatter();
        // nick,server,port,password,channel
        ...
        saveButton.addClickHandler(new ClickHandler()
        {
            ...
            public void onClick(ClickEvent event)
            {
                ...
                box.hide();
                BZNetwork.authLink.updateIrcBot(bot.getBotid(), nickField
                        .getText(), serverField.getText(), portNumber,
                        passwordField.getText(), channelField.getText(),
    ...
    {
        if (!Window
                .confirm("Are you sure you want to delete this IRC bot? This will "
                        + "also delete any triggers that use it as a recipient."))
            return;
    ...
    public static void resyncIrc()
    {
        BZNetwork.authLink.reconnectIrcBots(new BoxCallback<Void>()
        {
            ...
            {
                Window
                        .alert("IRC connections have been successfully re-synchronized.");
            }
        });
    }
}
