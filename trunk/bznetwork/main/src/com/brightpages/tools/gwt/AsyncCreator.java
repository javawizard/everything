...
public class AsyncCreator {
	...
	private static final Pattern returnTypeArg = Pattern.compile("@gwt.typeArgs\\s*<.*>");
	...
	public static void main(String[] args) throws Exception {
		if (args.length < 1) {
			log.log(Level.WARNING, "No arguments were passed. Pass the location of the package containing the RPC interfaces.");
			System.exit(-1);
		}
	...
	public void generateAsyncForPackage(File packageFolder) throws Exception {
		if (!packageFolder.isDirectory()) {
			throw new Exception(packageFolder.getAbsolutePath() + " is not a valid package folder.");
		}
		for (File file : packageFolder.listFiles()) {
			if (file.isFile() && file.getName().endsWith(".java") && !file.getName().endsWith("Async.java")) {
				String asyncVersion = createAsync(file);
				String nonAsyncPath = file.getAbsolutePath();
				File asyncFile = new File(nonAsyncPath.substring(0, nonAsyncPath.length() - ".java".length()) + "Async.java");
				if (asyncFile.isFile()) {
					asyncFile.delete();
				}
	...
	public String createAsync(File file) throws Exception {
		...
		if (compiler == null) {
			log.log(Level.SEVERE, "No system java compiler available. You may be using JRE. This tool only works with JDK 1.6.");
			System.exit(-1);
		}

		standardFileManager = compiler.getStandardFileManager(null, null, null);
		Iterable<? extends JavaFileObject> java = standardFileManager.getJavaFileObjectsFromFiles(Arrays.asList(file));

		CompilationTask task = compiler.getTask(null, standardFileManager, null, null, null, java);
		...
		for (CompilationUnitTree unit : parseTree) {
			...
			asyncVersion.append("import com.google.gwt.user.client.rpc.AsyncCallback;\n");
			...
			for (Tree t : unit.getTypeDecls()) {
				if (t instanceof ClassTree) {
					...
					asyncVersion.append("\n/**\n* Asynchronous interface for " + classTree.getSimpleName() + " generated by BrightPages(c) AsyncCreator\n*/\n");
					...
					asyncVersion.append("public interface " + classTree.getSimpleName() + "Async " + extendsStr + " { \n\n");
					...
					for (Tree member : classTree.getMembers()) {
						if (member.getKind().equals(Kind.METHOD)) {
							...
							if (comments != null) {
								...
								comments = returnTypeArg.matcher(comments).replaceAll("");
								asyncVersion.append("/**\n*" + comments.replaceAll("\n", "\n*") + "/\n");
							}

							// always return void for async
							asyncVersion.append("void " + method.getName());
							asyncVersion.append("(");
							for (VariableTree param : method.getParameters()) {
								asyncVersion.append(param.getType().toString() + " " + param.getName() + ", ");
							}
							String throwsExpr="";
							...
							// add async method
							asyncVersion.append("AsyncCallback callback)"+throwsExpr+";\n\n");
						} else {
