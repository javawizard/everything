...
public class OfflineMessageStore extends BasicModule implements UserEventListener
{
    ...
    private static final String LOAD_OFFLINE =
        "SELECT stanza, creationDate, messageID FROM ofOffline WHERE username=?";
    private static final String LOAD_OFFLINE_MESSAGE =
        "SELECT stanza, messageID FROM ofOffline WHERE username=? AND creationDate=?";
    private static final String LOAD_OFFLINE_MESSAGE_BY_ID =
        "SELECT stanza, messageID FROM ofOffline WHERE username=? AND messageID=?";
    private static final String LOAD_NEXT_MESSAGE_ID =
        "SELECT messageID FROM ofOffline WHERE username = ? and messageID > ? order by messageID asc limit 1";
    ...
    /*
     * OpenGroove G4 extension
     */
    private static final String DELETE_OFFLINE_MESSAGE_BY_CONTENT =
    ...
    public long addMessage(Message message)
    {
        ...
        else if (!XMPPServer.getInstance().getServerInfo().getXMPPDomain().equals(
            recipient.getDomain()))
        {
    ...
    public Collection<OfflineMessage> getMessages(String username, boolean delete)
    {
        ...
        try
        {
            ...
            while (rs.next())
            {
                ...
                {
                    message =
                        new OfflineMessage(creationDate, xmlReader.read(
                ...
                // OpenGroove G4 extension: we'll also set the subject of the
                // message to be the message's id. This discards the subject,
                // but we don't care about that since G4 doesn't make use of
                // the subject in its protocol.
                message.setSubject("" + messageID);
                Element delay = message.addChildElement("x", "jabber:x:delay");
                delay.addAttribute("from", XMPPServer.getInstance().getServerInfo()
                    .getXMPPDomain());
     ...
     *         stamp.
     */
    public OfflineMessage getMessageById(String username, long messageID)
    {
        OfflineMessage message = null;
        Connection con = null;
        PreparedStatement pstmt = null;
        ...
        try
        {
            ...
            xmlReader = xmlReaders.take();
            con = DbConnectionManager.getConnection();
            pstmt = con.prepareStatement(LOAD_OFFLINE_MESSAGE_BY_ID);
            pstmt.setString(1, username);
            pstmt.setLong(2, messageID);
            rs = pstmt.executeQuery();
            while (rs.next())
            {
                String msgXML = rs.getString(1);
                message =
                    new OfflineMessage(new Date(), xmlReader.read(
                        new StringReader(msgXML)).getRootElement());
                // Add a delayed delivery (JEP-0091) element to the message.
                // OpenGroove G4 extension: we'll also set the subject of the
                // message to be the message's id. This discards the subject,
                ...
                delay.addAttribute("from", XMPPServer.getInstance().getServerInfo()
                    .getXMPPDomain());
                delay.addAttribute("stamp", dateFormat.format(new Date()));
            }
        }
        catch (Exception e)
        {
            Log.error("Error retrieving offline messages of username: " + username
                + " creationDate: " + messageID, e);
        }
        finally
        {
            // Return the sax reader to the pool
    ...
    /**
     * Deletes any offline messages addressed to the node specified that contain
     * the content specified. This is an OpenGroove G4 extension to the server.
     * 
     * @param username
     *            The username (everything before the at sign) of the user that
     *            these messages to delete are for
     * @param content
     *            The content to search for in the messages. This is
     *            case-sensitive, and can appear anywhere in the message's
     *            contents for the message to be deleted. Additionally, the
     *            percent sign can be used as a wildcard character.
     */
    public void deleteMessageByContent(String username, String content)
    {
        Connection con = null;
        PreparedStatement pstmt = null;
        try
        {
            con = DbConnectionManager.getConnection();
            pstmt = con.prepareStatement(DELETE_OFFLINE_MESSAGE_BY_CONTENT);
            pstmt.setString(1, username);
            pstmt.setLong(2, Long.parseLong(content));
            pstmt.executeUpdate();
            // We don't know how many messages were deleted, so we'll just
            // invalidate the cache for this user and force a reload the next
            // time the cache is used.
            removeUsernameFromSizeCache(username);
        }
        catch (Exception e)
        {
            Log.error("Error deleting offline messages of username: " + username
                + " deletion content: " + content, e);
        }
        finally
        {
            DbConnectionManager.closeConnection(pstmt, con);
    ...
    public long loadNextMessageId(String username, long previousId)
    {
        Connection con = null;
        PreparedStatement pstmt = null;
        ResultSet rs = null;
        ...
        {
            con = DbConnectionManager.getConnection();
            pstmt = con.prepareStatement(LOAD_NEXT_MESSAGE_ID);
            pstmt.setString(1, username);
            pstmt.setLong(2, previousId);
            rs = pstmt.executeQuery();
            if (rs.next())
            {
                return rs.getLong(1);
