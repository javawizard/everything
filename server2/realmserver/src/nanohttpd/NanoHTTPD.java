...
public class NanoHTTPD
{
    ...
    public class Response
    {
            ...
            String txt)
        {
            this.status = status;
            this.mimeType = mimeType;
            this.data = new ByteArrayInputStream(txt
                .getBytes());
        }
    ...
    {
        System.out
            .println("NanoHTTPD 1.11 (C) 2001,2005-2008 Jarno Elonen\n"
        ...
        System.out.println("Now serving files in port "
            + port + " from \""
    ...
    private class HTTPSession implements Runnable
    {
            ...
            try
            {
                ...
                if (qmi >= 0)
                {
                    decodeParms(uri.substring(qmi + 1),
                        parms);
                    uri = decodePercent(uri.substring(0,
                        qmi));
                }
                else
                ...
                if (r == null)
                    ...
                    sendResponse(r.status, r.mimeType,
                        r.header, r.data);
            ...
            throws InterruptedException
        {
            ...
            while (st.hasMoreTokens())
            {
                ...
                if (sep >= 0)
                    p.put(
                        decodePercent(e.substring(0, sep))
        ...
        private void sendResponse(String status,
            String mime, Properties header, InputStream data)
        {
            try
            {
                if (status == null)
    ...
    public Response serveFile(String uri,
        Properties header, File homeDir,
        boolean allowDirectoryListing)
    {
        ...
        if (f.isDirectory())
        {
            ...
            {
                String[] files = f.list();
                String msg = "<html><body><h1>Directory "
                    + uri + "</h1><br/>";
                
                if (uri.length() > 1)
                {
                    String u = uri.substring(0, uri
                        .length() - 1);
        ...
        try
        {
            ...
            if (dot >= 0)
                mime = (String) theMimeTypes.get(f
                    .getCanonicalPath().substring(dot + 1)
                    .toLowerCase());
            if (mime == null)
            ...
            r.addHeader("Content-length", ""
                + (f.length() - startFrom));
            r
                .addHeader("Content-range", "" + startFrom
                    + "-" + (f.length() - 1) + "/"
                    + f.length());
            return r;
        }
