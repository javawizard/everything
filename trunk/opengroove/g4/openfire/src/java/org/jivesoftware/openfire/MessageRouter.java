...
public class MessageRouter extends BasicModule
{
        ...
        try
        {
            ...
            if (session == null || session.getStatus() == Session.STATUS_AUTHENTICATED)
            {
                ...
                try
                {
                    ...
                    // G4MOD: Instead of just routing it here, we should add it
                    // to the offline message list and then route it. Then the
                    // catch
                    /*
                     * If the user is registered and is on this server
                     * (OpenGroove G4 won't use resource names so no need to
                     * worry about that), then store the message as an offline
                     * message too.
                     */
                    if (serverName.equals(recipientJID.getDomain())
                        && userManager.isRegisteredUser(recipientJID.getNode()))
                    {
                        long mid = messageStrategy.storeOffline((Message) packet);
                        // OpenGroove G4 Extension: the subject of a message is
                        // essentially discarded, and replaced by the Openfire
                        // message id so that the client can delete the message.
                        packet.setSubject("" + mid);
                    }
     ...
     * @param packet
     *            Message packet that failed to be sent to the receipient.
     */
    public void routingFailed(JID receipient, Packet packet)
    {
        // If message was sent to an unavailable full JID of a user then retry
        // using the bare JID
        if (serverName.equals(receipient.getDomain())
            && receipient.getResource() != null
        ...
        {
            // The message will have already been stored
            // System.out
            // .println("Failed message routing without resource, forwarding to offline storage");
