...
public class RiskBoard extends JPanel
{
	...
	public synchronized void addPolygon(Polygon polygon, Color color,
			boolean fill, Stroke stroke)
	{
	...
	public RiskBoard()
	{
		...
		try
		{
			boardImage = ImageIO.read(getClass().getResourceAsStream(
					"worldmap.jpg"));
		} catch (IOException e)
		{
			// TODO Feb 15, 2008 Auto-generated catch block
			throw new RuntimeException("TODO auto generated on Feb 15, 2008 : "
	...
	{
		if (!(g7 instanceof Graphics2D))
			throw new RuntimeException(
					"RiskBoard requires a Graphics2D to function.");
		Graphics2D g = (Graphics2D) g7;
		g.setStroke(new BasicStroke(1.0f, BasicStroke.CAP_BUTT,
				BasicStroke.JOIN_ROUND));
		g.drawImage(boardImage, 0, 0, null);
		for (int i = 0; i < polygons.size() && i < colors.size()
				&& i < fills.size(); i++)
		{
