 ...
 * and components. As soon as a user authenticates with the server its client
 * session will be added to the routing table. Whenever the client session
 * becomes available or unavailable the routing table will be updated too.
 * <p>
 * 
 * When running inside of a cluster the routing table will also keep references
 * to routes hosted in other cluster nodes. A {@link RemotePacketRouter} will be
 * use to route packets to routes hosted in other cluster nodes.
 * <p>
 * 
 * Failure to route a packet will end up sending
 * {@link IQRouter#routingFailed(JID, Packet)},
 * {@link MessageRouter#routingFailed(JID, Packet)} or
 * {@link PresenceRouter#routingFailed(JID, Packet)} depending on the packet
 * type that tried to be sent.
 * 
    ...
    ClusterEventListener
{
    ...
    /**
     * Cache (unlimited, never expire) that holds outgoing sessions to remote
     * servers from this server. Key: server domain, Value: nodeID
     */
    private Cache<String, byte[]> serversCache;
    /**
     * Cache (unlimited, never expire) that holds components connected to the
     * server. Key: component domain, Value: list of nodeIDs hosting the
     * component
     */
    private Cache<String, Set<NodeID>> componentsCache;
    /**
     * Cache (unlimited, never expire) that holds sessions of user that have
     * authenticated with the server. Key: full JID, Value: {nodeID,
     * available/unavailable}
     */
    private Cache<String, ClientRoute> usersCache;
    /**
     * Cache (unlimited, never expire) that holds sessions of anonymous user
     * that have authenticated with the server. Key: full JID, Value: {nodeID,
     * available/unavailable}
     */
    private Cache<String, ClientRoute> anonymousUsersCache;
    /**
     * Cache (unlimited, never expire) that holds list of connected resources of
     * authenticated users (includes anonymous). Key: bare JID, Value: list of
     * full JIDs of the user
     */
    ...
    public boolean addClientRoute(JID route, LocalClientSession destination)
    {
        ...
        if (destination.getAuthToken().isAnonymous())
        {
            ...
            try
            {
                lockAn.lock();
                added =
                    anonymousUsersCache.put(route.toString(), new ClientRoute(server
                        .getNodeID(), available)) == null;
            }
            finally
            {
                lockAn.unlock();
            ...
            if (route.getResource() != null && (!available || added))
            {
                ...
                {
                    lock.lock();
                    usersSessions.put(route.toBareJID(), Arrays
                        .asList(route.toString()));
                }
                finally
                {
                    lock.unlock();
        ...
        else
        {
            ...
            try
            {
                lockU.lock();
                added =
                    usersCache.put(route.toString(), new ClientRoute(
                        server.getNodeID(), available)) == null;
            }
            finally
            {
                lockU.unlock();
            ...
            if (route.getResource() != null && (!available || added))
            {
                ...
                try
                {
                    ...
                    if (jids == null)
                    {
                        // Optimization - use different class depending on
                        // current setup
    ...
    public void broadcastPacket(Message packet, boolean onlyLocal)
    {
        ...
        // Check if we need to broadcast the message to client sessions
        // connected to remote cluter nodes
    ...
    public void routePacket(JID jid, Packet packet, boolean fromServer)
        throws PacketException
    {
        ...
        if (serverName.equals(jid.getDomain()))
        {
            if (jid.getResource() == null)
            {
                ...
                else
                {
                    throw new PacketException(
                        "Cannot route packet of type IQ or Presence to bare JID: "
                            + packet);
                }
            }
            else
            {
                ...
                if (clientRoute != null)
                {
                    if (!clientRoute.isAvailable()
                        && routeOnlyAvailable(packet, fromServer)
                        && !presenceUpdateHandler.hasDirectPresence(packet.getTo(),
                            packet.getFrom()))
                    {
                        // Packet should only be sent to available sessions and
                    ...
                    else
                    {
                        ...
                        {
                            // This is a route to a local user hosted in this
                            // node
                            try
                            {
                                localRoutingTable.getRoute(jid.toString()).process(
                                    packet);
                                routed = true;
                            }
                        ...
                        {
                            // This is a route to a local user hosted in other
                            // node
                            if (remotePacketRouter != null)
                            {
                                routed =
                                    remotePacketRouter.routePacket(clientRoute
                                        .getNodeID().toByteArray(), jid, packet);
                            }
                        }
                    }
                }
            ...
            && (hasComponentRoute(jid) || ExternalComponentManager.hasConfiguration(jid
                .getDomain())))
        {
            ...
            else
            {
                ...
                if (nodes != null)
                {
                    for (NodeID nodeID : nodes)
                    {
                        ...
                        {
                            // This is a route to a local component hosted in
                            // this node (route
                            // could have been added after our previous check)
                            try
                            {
                                localRoutingTable.getRoute(jid.getDomain()).process(
                                    packet);
                                routed = true;
                                break;
                            }
                        ...
                        else
                        {
                            ...
                            {
                                routed =
                                    remotePacketRouter.routePacket(
                                        nodeID.toByteArray(), jid, packet);
                                if (routed)
                                {
                                    break;
                                }
        ...
        else
        {
            ...
            if (nodeID != null)
            {
                if (server.getNodeID().equals(nodeID))
                {
                    // This is a route to a remote server connected from this
                    // node
                    try
                    {
                ...
                {
                    // This is a route to a remote server connected from other
                    // node
                    if (remotePacketRouter != null)
                    {
            ...
            {
                // Return a promise of a remote session. This object will queue
                // packets pending
        ...
        if (!routed)
        {
            ...
            {
                Log.debug("RoutingTableImpl: Failed to route packet to JID: " + jid
                    + " packet: " + packet);
            }
    ...
    /**
     * Returns true if the specified packet must only be route to available
     * client sessions.
     * 
     * @param packet
     *            the packet to route.
     ...
     * @return true if the specified packet must only be route to available
     *         client sessions.
     */
    private boolean routeOnlyAvailable(Packet packet, boolean fromServer)
    {
        if (fromServer)
        {
            // Packets created by the server (no matter their FROM value) must
            // always be delivered no
        ...
        if (packet instanceof IQ)
        {
            onlyAvailable =
                hasSender
                    && !(serverName.equals(from.getDomain()) && from.getResource() == null)
        ...
        else if (packet instanceof Message || packet instanceof Presence)
        {
            onlyAvailable =
                !hasSender
                    || (!serverName.equals(from.toString()) && !componentsCache
                        .containsKey(from.getDomain()));
        }
        return onlyAvailable;
    }
    
    /**
     * Deliver the message sent to the bare JID of a local user to the best
     * connected resource. If the target user is not online then messages will
     * be stored offline according to the offline strategy. However, if the user
     * is connected from only one resource then the message will be delivered to
     * that resource. In the case that the user is connected from many resources
     * the logic will be the following:
     * <ol>
     * <li>Select resources with highest priority</li>
     * <li>Select resources with highest show value (chat, available, away, xa,
     * dnd)</li>
     * <li>Select resource with most recent activity</li>
     * </ol>
     * 
     * Admins can override the above logic and just send the message to all
     * connected resources with highest priority by setting the system property
     * <tt>route.all-resources</tt> to <tt>true</tt>.
     * 
     * @param recipientJID
     *            the bare JID of the target local user.
     * @param packet
     *            the message to send.
    ...
    private boolean routeToBareJID(JID recipientJID, Message packet)
    {
        ...
        // Get existing AVAILABLE sessions of this user or AVAILABLE to the
        ...
        else
        {
            ...
            if (!JiveGlobals.getBooleanProperty("route.all-resources", false))
            {
                ...
                // Get session with most recent activity (and highest show
                // value)
                ...
                // Deliver stanza to session with highest priority, highest show
                // value and most recent activity
                targets.get(0).process(packet);
            }
            else
            {
                // Deliver stanza to all connected resources with highest
                // priority
                for (ClientSession session : sessions)
                {
                    session.process(packet);
    ...
    /**
     * Returns the sessions that had the highest presence priority greater than
     * zero.
     * 
     * @param sessions
     *            the list of user sessions that filter and get the ones with
     *            highest priority.
     * @return the sessions that had the highest presence priority greater than
     *         zero or empty collection if all were negative.
     */
    ...
    public ClientSession getClientRoute(JID jid)
    {
        ...
        ClientSession session =
            (ClientSession) localRoutingTable.getRoute(jid.toString());
        if (session == null)
        {
            ...
            if (locator != null)
            {
                ...
                {
                    session =
                        locator.getClientSession(route.getNodeID().toByteArray(), jid);
                }
            }
        }
        return session;
    ...
    public Collection<ClientSession> getClientsRoutes(boolean onlyLocal)
    {
        ...
        Collection<ClientSession> sessions =
            new ArrayList<ClientSession>(localRoutingTable.getClientRoutes());
        if (!onlyLocal)
        {
            ...
            if (locator != null)
            {
                // Add sessions of non-anonymous users hosted by other cluster
                // nodes
                for (Map.Entry<String, ClientRoute> entry : usersCache.entrySet())
                {
                    ...
                    {
                        sessions.add(locator.getClientSession(route.getNodeID()
                            .toByteArray(), new JID(entry.getKey())));
                    }
                }
                // Add sessions of anonymous users hosted by other cluster nodes
                for (Map.Entry<String, ClientRoute> entry : anonymousUsersCache
                    .entrySet())
                {
                    ClientRoute route = entry.getValue();
                    if (!server.getNodeID().equals(route.getNodeID()))
    ...
    public OutgoingServerSession getServerRoute(JID jid)
    {
        ...
        OutgoingServerSession session =
            (OutgoingServerSession) localRoutingTable.getRoute(jid.getDomain());
        if (session == null)
        {
            // The session is not in this JVM so assume remote
            RemoteSessionLocator locator = server.getRemoteSessionLocator();
    ...
    public List<JID> getRoutes(JID route, JID requester)
    {
        ...
        if (serverName.equals(route.getDomain()))
        {
            ...
            if (route.getResource() != null)
            {
                ...
                if (clientRoute != null
                    && (clientRoute.isAvailable() || presenceUpdateHandler
                        .hasDirectPresence(route, requester)))
                {
                    jids.add(route);
                }
            }
            ...
            {
                // Address is a bare JID so return all AVAILABLE resources of
                // user
                ...
                if (sessions != null)
                {
                    ...
                    for (String jid : sessions)
                    {
                        ...
                        if (clientRoute != null
                            && (clientRoute.isAvailable() || presenceUpdateHandler
                                .hasDirectPresence(new JID(jid), requester)))
                        {
    ...
    public void joinedCluster()
    {
        ...
        // Broadcast presence of local sessions to remote sessions when
        // subscribed to presence
        // Probe presences of remote sessions when subscribed to presence of
        // local session
        // Send pending subscription requests to local sessions from remote
        // sessions
        // Deliver offline messages sent to local sessions that were unavailable
        // in other nodes
        // Send available presences of local sessions to other resources of the
        // same user
    ...
    {
        // Add outgoing server sessions hosted locally to the cache (using new
        // nodeID)
        ...
        // Add component sessions hosted locally to the cache (using new nodeID)
        // and remove traces to old nodeID
